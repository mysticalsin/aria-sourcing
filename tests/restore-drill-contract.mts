...
ok("restore verifies exact migration versions", /EXPECTED_MIGRATIONS/.test(restore) && /ACTUAL_MIGRATIONS/.test(restore));
...
ok("backup requires the exact migration set", /EXPECTED_MIGRATIONS/.test(backup) && /ACTUAL_MIGRATIONS/.test(backup));
...
const backupCreate = backup.indexOf('dex -d postgres -c "create database');
const restoreArm = restore.indexOf("SCRATCH_CLEANUP_ARMED=1");
const restoreCreate = restore.indexOf('dex -d postgres -c "create database');
...
for (const table of [
  ...
  "email_connections", "messages_inbound", "messages_outbound", "outbound_content_cache",
