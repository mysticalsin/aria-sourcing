...
test("manifest preserves parity and freezes the exact deduplicated lifecycle", () => {
  assert.deepEqual(
    ...
    { pretest: 51, application: 135, posttest: 2, all: 188 },
  );
  ...
  assert.equal(
    ...
    "443fc57934fb98fcd544f9c5d79f759993d54c438c97f078b9db2b2e71ba8220",
  );
  ...
  assert.equal(parityLines.length, 190);
  assert.equal(
    createHash("sha256").update(parityLines.join("\n")).digest("hex"),
    "8f1eef72b63c5423022e9120a0cfc70fe97bd46156a0f1129e23d79534672106",
...
test("named manifest groups freeze their recursive baselines", () => {
  const expected = {
    ...
    database: [12, "b666bf725177df446135de939d93f9d99e7cdf5b0ac97a67f6df1603eef6ad3f"],
