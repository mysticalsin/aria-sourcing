...
// --- mapApifyCandidates: scored, deduped Candidates with compliance ---------
{
  ...
  ok("sourcePlatform is LinkedIn (operator-facing; never Apify)", c?.sourcePlatform === "LinkedIn");
  ok("Apify external id preserved for enrichment identity", c?.externalIds?.Apify === "test-candidate-dev");
  ...
  ok("compliance note records LinkedIn profile search provenance", note.includes("LinkedIn profile search") && note.toLowerCase().includes("third-party"));
  ...
  const legacy = mapApifyCandidates([profile], campaign, "q", [], W, { displayPlatform: "Apify" });
