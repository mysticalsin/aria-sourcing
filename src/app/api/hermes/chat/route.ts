...
export async function POST(req: NextRequest) {
  ...
  if (provider !== "hermes") {
    ...
    if (task === "chat" && slug !== "kimi" && (webResearch || usableMcpServers || sourcingCampaign)) {
      ...
      const apifyToken = canSourceInChat && supabase ? await resolveStoredApifyKey(supabase) : null;
      ...
      // Compliant sourcing tool: real search (LinkedIn web + Apify + GitHub),
      // real dedupe, real deterministic scoring — never a stealth browser.
      if (sourcingCampaign) {
