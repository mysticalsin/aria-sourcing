...
function scoreSkills(c: Candidate, jd: JobAnalysis): { score: number; rationale: string } {
  ...
  if (c.sourcePlatform === "GitHub" && c.provenance !== "synthetic") {
    ...
    else if (reqHit >= 2) score = Math.max(score, 86);
    else if (reqHit === 1 && req.length <= 2 && corpus.replace(/\s+/g, " ").trim().length >= 120) {
  ...
  if (!hasGeo) {
    ...
    if (europeFocus) {
      if (timezoneAligned || europeHit || regionAligned || targetPlaceHit) {
        ...
        geoScore = 58;
        geoRationale = "Remote Europe/EMEA role: European timezone/location not confirmed";
      }
    } else {
      geoScore =
