    ...
    <Card id={EMAIL_CONNECTIONS_PANEL_ID} className="overflow-hidden border-line/80 bg-surface shadow-sm">
      <CardContent className="space-y-5 p-6 sm:p-8">
            ...
            {providers && (!providers.microsoftOAuth || !providers.encryptionReady) ? (
              <p className="max-w-md text-xs text-muted">
                {!providers.encryptionReady
                  ...
                  : "Microsoft Graph OAuth env missing — Entra admin must register ARIA Mantu Graph (Fly), Owners → Add twalteur@amaris.com, Grant admin consent, then set MICROSOFT_CLIENT_ID/SECRET/TENANT_ID on Fly (or drop client id for agent apply). Connect Outlook stays disabled. HMAC mailbox above still works for intake."}
              </p>
            ) : null}
          </div>
        )}
        ...
        <p className="flex items-start gap-1.5 text-xs text-muted">
          ...
          (nothing auto-sends unless Autopilot is entitled and Sequences are armed). Approve → Send uses your Outlook mailbox for Email. LinkedIn uses
          HeyReach when configured (Autopilot ON auto-queues; OFF still Approve → Send); without HeyReach it stays copy/paste + confirm.
          Emergency inbox sync on Intake is break-glass only. See{" "}
