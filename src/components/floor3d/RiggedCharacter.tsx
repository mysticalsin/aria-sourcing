...
/* ============================================================================
   ...
   hermes office RiggedCharacter: SkeletonUtils clone, auto-scale to a fixed
   height, optional per-instance tint, and idle/walk/sit clip crossfade driven
   off the agent's render state. No vite `?url` imports — the GLB is served from
   /public.
   ========================================================================== */

export const RIGGED_MAN_URL = "/office3d/assets/biz_man.glb";
...
export function RiggedCharacter({
  ...
  const { clonedScene, clonedMaterials } = useMemo(() => {
    ...
    // Track every material produced by mat.clone() so we can dispose them on
    // unmount (or when tint/scene changes). We must NOT dispose the originals —
    ...
    return { clonedScene: cloned, clonedMaterials: clonedMats };
  }, [scene, tint]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Dispose only the materials that were produced by mat.clone() (tracked in
      // clonedMaterials). Shared GLTF originals are never touched.
      for (const m of clonedMaterials) m.dispose();
    };
  }, [mixer, clonedScene, clonedMaterials]);
  ...
  useFrame((_, delta) => {
    ...
    if (agent.state === "walking") {
      targetClipIdx = (agent.walkSpeed ?? 0) > 2.5 ? clipMap.sprint : clipMap.walk;
...
useGLTF.preload(RIGGED_MAN_URL);
