...
gen_draft() {  # $1 = channel label used only in the prompt
  ...
  prompt="Draft a first-touch ${channel} recruiting message in language ISO code ${E2E_OUTREACH_LANGUAGE}. The candidate's main language is ${E2E_LANG_LABEL}. Write the entire message in ${E2E_LANG_LABEL} only (proper nouns like Mantu Group excepted). Do not ask clarifying questions — output the final message only. ${fmt_hint} Reach out to ${CAND_NAME} about a senior engineering role with Mantu Group. Mention Mantu Group by name. Lead with ONE specific human detail from this candidate profile (project/repo name, architecture detail, or stack — not a generic compliment, and not an employer/company-name compliment alone):${specifics} If a company is listed, pair it with a concrete tech/project detail or skip the company name in the opener. Soft ask for a short chat. Avoid salary/compensation. Avoid generic openers like 'I hope this finds you well'. Never cite raw scraped metrics (public repo counts, follower counts, star totals) — that reads as automated and impersonal."
