...
| Discrimination-proxy filter | Provider-backed sourcing egress requires a `ProviderClearance`; discovery clearances are minted only after criteria are rejected for age/gender/race/religion/disability/marital/nationality/university-graduation proxies and required to bind to the approved role's own JD terms | `src/lib/sourcing/query-policy.ts`, `src/lib/sourcing/provider-egress.ts` |
