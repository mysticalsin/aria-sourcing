...
declare
  ...
  expected_functions constant text :=
    'audit_databricks_connection_authority(),audit_dust_connection_authority(),claim_and_record(text,text,text,uuid,text,integer),claim_email_outbound(text,text,text,text,text,text,uuid),claim_whatsapp_inbound_processing(uuid,uuid),claim_whatsapp_outbound(uuid),complete_whatsapp_inbound_processing(uuid,uuid,text,text),create_agent_run_with_memory_context(uuid,uuid,uuid,uuid),current_profile_role(),current_workspace_id(),enforce_active_whatsapp_approval(),enforce_agent_memory_authority_immutable(),enforce_agent_run_authority_immutable(),enforce_agent_spec_authority_immutable(),ensure_workspace(),finalize_whatsapp_provider_failure(uuid,uuid,text),normalize_whatsapp_e164(text),record_outreach_approval(text,text,text),record_whatsapp_delivery_event(uuid,uuid,text,text,timestamp with time zone,integer),record_whatsapp_provider_acceptance(uuid,uuid,text),reject_agent_memory_audit_mutation(),resolve_whatsapp_inbound_conversation(uuid,uuid),review_whatsapp_outbound(uuid,text),revoke_outreach_approval(text,text),stamp_databricks_connection_authority(),stamp_dust_connection_authority(),strip_legacy_agent_memory_authority(),strip_legacy_databricks_authority(),strip_legacy_dust_authority(),touch_updated_at()';
begin
