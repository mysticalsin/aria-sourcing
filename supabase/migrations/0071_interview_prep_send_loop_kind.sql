-- 0071_interview_prep_send_loop_kind.sql
--
-- Queue interviewer prep + candidate confirmation drafts after live booking.
...
begin
  ...
  case p_kind
    ...
    when 'interview_prep_send' then allowed_keys := array['campaignId', 'candidateId', 'bookingId', 'trigger'];
  ...
  select coalesce(auth.role(), '') = 'service_role'
     and coalesce((
      select case
        ...
        when p_kind in (
          'calendar_book', 'pre_call_propose', 'first_interview_book', 'interview_prep_send',
...
begin
  ...
  if p_workspace_id is null
     ...
     or p_kind not in (
       ...
       'interview_prep_send', 'delivery_reconcile', 'outcome_feedback', 'swarm_assignment'
     )
...
comment on function public.enqueue_aria_job(uuid, text, text, jsonb, timestamptz, integer) is
  'Enqueue an aria_jobs row. Kinds include interview_prep_send (0071) for post-booking prep drafts.';
