-- 0068_apply_workspace_patch_digest_path.sql
--
-- Live Fly (2026-08-27): requisition_parse → campaign materialization failed with
-- handler:requisition_parse:rpc_http_404. PostgREST returned HTTP 404 wrapping
-- Postgres 42883: function digest(text, unknown) does not exist.
--
-- Root cause: 0063_loop_append_outreach.sql rewrote apply_workspace_patch with
--   set search_path = pg_catalog, public, pg_temp
-- dropping `extensions` that 0042 included. On this tenant pgcrypto's digest()
-- lives in schema `extensions` (create extension IF NOT EXISTS into public is a
-- no-op when the extension already exists elsewhere), so SECURITY DEFINER cannot
-- resolve digest. The 'sha256' literal is also typed as unknown without a cast.
--
-- Fix: keep 0063 behavior (append_outreach + not_found), restore extensions on
-- search_path, schema-qualify digest with an explicit text cast, and fall back
-- public → extensions → double-md5 (64 hex) so receipts never throw 42883.
...
declare
  ...
  append_kinds constant text[] := array[
    ...
    'append_enrichment_ledger', 'append_outreach'];
  append_key text;
begin
  ...
  exception
    when undefined_function then
      ...
      exception
        when undefined_function then
          -- Always-available 64-hex fallback (md5 is in pg_catalog). Only used when
          -- pgcrypto is absent from both public and extensions — keeps D1 writable.
