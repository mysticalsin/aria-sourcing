...
cd "$(dirname "$0")"
repo="$(pwd)"
# Release authority BEFORE any credential load or Fly mutation. This is an
# owner-run emergency resume surface, so it is allowed to exist only when bound
# to an exact reviewed SHA, a clean tree, an explicit app-naming confirmation and
# a local receipt — the same contract the scripts/prod-*.sh surfaces carry.
source "$repo/scripts/lib/prod-release-guard.sh"
aria_require_reviewed_production_release deploy-fly-2 \
  aria-mantu-db aria-mantu-kong aria-mantu-bootstrap aria-mantu-app
