...
shift: 186
agent: cursor-cloud
updated: 2026-09-12T01:45Z
status: ready-retarget-floor-send-fail-closed-tip-not-released
---
...
1. `ensureComputer` treats `status=ready` as live — refuses silent retarget before session probe
2. `navigate` / `session_probe` always drive ensured `computerId` (never raw request id after rebound)
3. Floor pulse + `pickResponderIndex` fail-closed without seatId (no hash desk paint)
4. `agentActivityWithComputers`: non-LI desks idle under live computers map unless `sentToday>0`
5. Aria Live prefers sole/assigned campaign seat — never hash-picks a desk
6. Send path: WA/SMS/Email fail-closed unless unique live provider or `msg.seatId` matches
7. Deleted unused theatrical floor3d modules (~920 LOC): CityWorld, OfficeRoom, InstancedAgents, OfficeFurniture
8. Tests: computer-supervisor 75/75; floor 63/63; sourcing-automatic-deliver 12/12; agent-event-seat 5/5; typecheck green
...
- Ambiguous N-seat send pick ⇒ fail-closed (LI/WA/SMS/Email)
