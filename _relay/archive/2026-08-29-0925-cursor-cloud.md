...
- **Live Fly:** `e5c37c1` / **0074** · `deploy_status=tip_ahead_docs` (script/docs tip `f13bf6c`+; no app remint required for this shift)
...
1. Aligned `scripts/verify-m365-ready.sh` with Graph-only strict E2E PASS:
   - Graph secrets + live seat (webhook + **Calendars.ReadWrite** + **OnlineMeetings.ReadWrite** + mode=live) remain fail-closed
   - Entra GoTrue Azure secrets + `/login` SSO → **WARN only** (no exit 6)
   - Fly-env LLM auth (`probe-fly-llm-auth`) → **WARN only** (no exit 7; Hermes/vault may green E2E)
2. Updated audit matrix pin for verify-m365-ready → still **64/64**
3. Did **not** run verify-m365-ready / strict M365 E2E while Microsoft deferred

## Blockers

- Owner reopen Microsoft for RESULT: PASS (Graph CLIENT/SECRET/TENANT dropzone → apply → Connect Outlook with Calendars+OnlineMeetings → `bash scripts/verify-m365-ready.sh` or strict E2E without PARTIAL_M365)

## Next steps

```bash
unset AGENT_PROVIDER AGENT_MODEL
bash scripts/print-fly-golive-status.sh   # tip_ahead_docs OK for scripts; live e5c37c1 / 0074
...
#   # or: unset ARIA_ALLOW_PARTIAL_M365_E2E; bash e2e-workflow-test.sh
```
...
- tip_ahead_docs after script-only commits — do not remint Fly for verify-m365-ready alone
