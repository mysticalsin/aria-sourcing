...
- Wired `docs-truth.mts` into the chain as the 98th suite command and committed the
  ...
  is now tracked). Bumped the suite count 97 to 98 in both `STATUS.md` and the
  docs-truth assertion so they stay coupled.
- Full gate green, verified in this session (not diff-read):
  - `npx tsc --noEmit` -> No errors.
  - Full `npm test` chain (pretest+test), run with the sandbox `tsx` to `node --import tsx`
    rewrite -> 98 suite commands, every `RESULT` reported 0 failed, chain exit 0
